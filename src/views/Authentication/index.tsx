...
export default function Authentication() {
  ...
  const SignInCard = () => {
    ...
    const [email, setEmalil] = useState<string>(signInMock.email);
      ...
      <div className="auth-card">
        <div className="auth-card-top">
          ...
          <div className="auth-card-top-input-container">
            <InputBox label='이메일 주소' type='text' placeholder='이메일 주소를 입력해주세요' error={error} value={email} setValue={setEmalil} />
  ...
  const SignUpCard = () => {
    ...
    const [email, setEmalil] = useState<string>('');
    ...
    //         function         //
    // description: 페이지1에서 페이지 2로 이동 시 검증 함수 //
    const checkPage1 = () => {
    ...
    // description: 로그인화면 이동 버튼 클릭 이벤트 //
          ...
          <div className="auth-card-top-input-container">
            {
              page === 1 ? (
                <>
                <InputBox label='이메일 주소*' type='text' placeholder='이메일 주소를 입력해주세요' error={emailPatternError || emailDuplicationError} helper={emailPatternError ? '이메일 주소 포맷이 맞지않습니다.' : emailDuplicationError ? '중복되는 이메일 주소 입니다.' : ''} value={email} setValue={setEmalil} />
                ...
                <InputBox label='주소*' type='text' placeholder='우편번호 찾기' icon={INPUT_ICON.ARROW} error={addressError} helper={addressError ? '우편번호를 선택해주세요.' : ''} value={address} setValue={setAddress} buttonHandler={onAddressIcnClickHandler} />
    ...
    <div id="auth-wrapper">
      <div className="auth-left">
        ...
        <div className="auth-left-text-container">
          ...
          <div className="auth-left-text">DONGS BOARD 입니다.</div>
        </div>
      </div>
