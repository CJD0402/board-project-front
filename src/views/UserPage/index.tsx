...
export default function UserPage() {
  ...
  const MyPageTop = () => {
    ...
    const patchNicknameRespnseHandler = (code: string) => {
      if (!user) return;
      if (code === 'NU') alert('존재하지 않는 유저입니다.');
    ...
    const onNicknameButtonClickHandler = () => {
      if (nicknameChange) {
        ...
        const token = cookie.accessToken;
        patchNicknameRequest(data, token).then(patchNicknameRespnseHandler);
      }
    ...
    //         effect          //
    // description: 유저 이메일 상태가 바뀔 때 마다 실행 //
    useEffect(() => {
  ...
  const { totalPage, currentPage, currentSection, onPreviusClickHandler, onNextClickHandler, onPageClickHandler, changeSection } = usePagination();
  ...
  //         effect          //
  // description: 유저 이메일이 바뀔 때 마다 유저 정보 및 게시물 리스트 불러오기 //
  useEffect(() => {
    if (!userEmail) {
  ...
  // description: 현재 페이지가 바뀔 때 마다 마이페이지 게시물 분류하기 //
  useEffect(() => {
