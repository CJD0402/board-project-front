...
import defaultProfileImageUrl from 'src/assets/default-profile-image.png';

//          component          //
// description: 게시물 상세 화면 //
export default function BoardDetail() {
  //          state          //
  // description: 해당 게시물 번호 상태 //
  const { boardNumber } = useParams();
  // description: 로그인 유저 정보 상태 //
  const { user } =useUserStore();
  // description: 페이지네이션 관련 상태 및 함수 //
  const { totalPage, currentPage, currentSection, onPreviusClickHandler, onPageClickHandler, onNextClickHandler, changeSection } = usePagination();
  // description: 쿠키 상태 //
  ...
  const [showFavoriteList, setShowLiktList] = useState<boolean>(false);
  ...
  const Board = () => {
    ...
    // description: 좋아요 리스트 펼치기 버튼 클릭 이벤트 //
    const onShowFavoriteListButtonClickHandler = () => {
      setShowLiktList(!showFavoriteList)
    }
    // description: 댓글 리스트 펼치기 버튼 클릭 이벤트 //
      ...
      <div className='board-detail-container'>
        <div className='board-detail-top'>
          ...
          <div className='board-detail-meta-left'>
              <div className='board-detail-writer-profile-image' style={{ backgroundImage: `url(${board?.writerProfileImage ? board.writerProfileImage : defaultProfileImageUrl })` }}></div>
    ...
    return (
      <div className="comment-list-box">
        ...
        <div className="comment-box">
          <textarea className='comment-textarea' rows={3} placeholder='댓글을 작성해주세요.' value={comment} onChange={onCommentChangeHandler}></textarea>
  ...
  // description: 현재 페이지가 바뀔 때 마다 검색 게시물 분류 //
  useEffect(() => {
    getPageCommentList(commentList);
  }, [currentPage]);
  // description: 현재 페이지가 바뀔 때 마다 페이지 리스트 변경 //
  useEffect(() => {
    changeSection(commentList.length, COUNT_BY_PAGE_COMMENT);
  }, [currentSection]);
